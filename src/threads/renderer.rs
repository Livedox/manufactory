...
pub fn spawn(
    ...
    thread::spawn(move || {loop {
        ...
        let chunk_position = world.get_mut().chunks.find_pos_stable_xyz(
            &|c| c.map_or(false, |c| c.modified()));

        if world.get_mut().chunks.is_translate {continue};
        if let Some(chunk_index) = chunk_position.map(|cp| cp.chunk_index(&world.get_mut().chunks)) {
            world.get_mut().chunks.chunks[chunk_index].as_mut().unwrap().modify(false);
            // Why with sleep it work better ????????????
            thread::sleep(Duration::from_millis(2));
            if let Some(indx) = results.iter().position(|a| {a.chunk_index == chunk_index}) {
                results.remove(indx);
            }
            if let Some(r) = render(chunk_index, world.get_mut()) {
                results.push(r);
            }
            results.sort_by(|a, b| {
                let a = world.get_mut().chunks.chunks[a.chunk_index].as_ref()
                    .map(|c| c.xyz)
                    .map_or(i32::MAX, |a| (a.0-pc.0).abs() + (a.1-pc.1).abs() +(a.2-pc.2).abs());
                let b = world.get_mut().chunks.chunks[b.chunk_index].as_ref()
                    .map(|c| c.xyz)
